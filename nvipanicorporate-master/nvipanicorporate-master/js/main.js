...
function initialize_map() {
    ...
    var markerlocations = {
        "marker":[{
            "lat":12.9718146802915,
            "lng":77.6542568302915,
            "title":'nVipani Technology Solutions Private Limited',
            "address":"854/1, NMS Arcade, 1st Floor, 8th B Cross, 6th Main Rd, Opp LIC Colony, New Tippasandra, Bengaluru, Karnataka-560075",
            "icon":'../img/mapmarker.png',
            "location":'https://goo.gl/maps/V2YdzesGEqr'
        },{
            "lat":39.027445,
            "lng":-77.5396127,
...
var i=0;
$(document).ready(function() {
 /*   $(".nvc-remove-news").click(function () {
        $(".nvc-remove-news").hide().siblings().show();
    });
    $(".nvc-remove-news1").click(function () {
        $(".nvc-remove-news1").hide().siblings().show();
    });
    $(".nvc-remove-news2").click(function () {
        $(".nvc-remove-news2").hide().siblings().show();
    });
    $(".nvc-remove-news3").click(function () {
        $(".nvc-remove-news3").hide().siblings().show();
    });*/
    $(".descriptionOfTeam").css("display", "none");
    $(".descriptionOfEmployee").css("display", "none");
    var team = [{
            "id": 1,
            "name": "dilli",
            "desination": "ceo",
            "description": "sometext",
            image: "img/aboutus/raghu.jpg"
        },
        {   "id": 2,
            "name": "hema",
            "desination": "ceo1",
            "description": "sometext",
            image: "img/aboutus/mentor.jpg"
        },
        {
            "id": 3,
            "name": "abc",
    ...
    function teamContent() {
        for (i = 0; i < team.length; i++) {
            var eachPlayer = "<div class='col-md-4 col-12 nvc-playerCards mb-5' id=" + team[i].name + ">"+"<img class='img-fluid' src=" + team[i].image + ">"+"<div>" + team[i].name + "</div>"+ "<div>" + team[i].desination + "</div>"+"</div>";
            $(".mentor-img-link").append(eachPlayer);
        }
    }
    teamContent();
    ...
    function employees() {
        for (i = 0; i < employee.length; i++) {
            var eachemployee = "<div class='col-md-4 col-12 nvc-playerCards mb-5' id=" + employee[i].name + ">"+"<img class='img-fluid' src=" + employee[i].image + ">"+"<div>" + employee[i].name + "</div>"+ "<div>" + employee[i].desination + "</div>"+"</div>";
            $(".team-img-link").append(eachemployee);
        }
    }
    employees();

    $('.mentor-img-link>div').on('click', function () {
        for(i=0;i<team.length;i++) {
            if(this.id===team[i].name) {
                var name = "<div>" + team[i].name +"</div>";
                var desination = "<div>" + team[i].desination +"</div>";
                var description = "<div>" + team[i].description +"</div>";
                var img = "<img class='img-fluid' src=" + team[i].image + ">";
                $(".total-content").css("display", "none");
                $(".descriptin-name").append(name);
                $(".descriptin-designation").append(desination);
                $(".descriptionTeam").append(description);
                $(".teamImage").append(img);
                $(".descriptionOfTeam").css("display", "block")
            }
        }
    });

    $('.team-img-link>div').on('click', function () {
        console.log(this.id);
        for(i=0;i<employee.length;i++) {
            if(this.id===employee[i].name) {
                var name = "<div>" + employee[i].name +"</div>";
                var desination = "<div>" + employee[i].desination +"</div>";
                var description = "<div>" + employee[i].description +"</div>";
                var img = "<img class='img-fluid' src=" + employee[i].image + ">";
                $(".total-content").css("display", "none");
                $(".descriptinnameEmployee").append(name);
                $(".descriptin-designationEmployee").append(desination);
                $(".descriptionTeamEmployee").append(description);
                $(".employeeImg").append(img);
                $(".descriptionOfEmployee").css("display", "block")
            }
        }
    });
});
...
function goBack() {
    ...
    $(".descriptin-name").empty();
    $(".descriptin-designation").empty();
    $(".descriptionTeam").empty();
    $(".teamImage").empty();
    $(".descriptinnameEmployee").empty();
    $(".descriptin-designationEmployee").empty();
    $(".descriptionTeamEmployee").empty();
    $(".employeeImg").empty();
}
